...
const DEFAULT_WORDS = {
  ...
  Medium: [
    'BADGE', 'BADLY', 'BAGEL', 'BAKER', 'BANJO', 'BARON', 'BEACH', 'BEAST',
    'BLANK', 'BOARD', 'BOOST', 'BOOTH', 'BOXER', 'BRAIN', 'BRAND', 'BRAVE',
    'BREAD', 'BREAK', 'BREED', 'BRIEF', 'BRING', 'BROAD', 'BROKE', 'BROWN',
    'BUILD', 'BULK', 'BUYER', 'CABLE', 'CABIN', 'CAGED', 'CAMEL', 'CANAL'
  ],
  Hard: [
    'DAZED', 'DEALT', 'DEATH', 'DECAL', 'DECAY', 'DECOR', 'DEFER', 'DEITY',
    'DELAY', 'DELTA', 'DELVE', 'DEMON', 'DENSE', 'DEPOT', 'DEPTH', 'DERBY',
    'DETER', 'DETOX', 'DEUCE', 'DIARY', 'DICED', 'DICER', 'DICKY', 'DICOT',
    'DIGIT', 'DILLY', 'DIMLY', 'DINER', 'DINKY', 'DIODE', 'DIOPT', 'DIPSY'
  ],
  Extreme: [
    'FJORD', 'GLYPH', 'PSYCH', 'NYMPH', 'LYMPH', 'MYRRH', 'MYTHS', 'PROXY',
    'CRWTH', 'WRYLY', 'SYZYGY', 'WHIZZ', 'JAZZY', 'FUZZY', 'DIZZY', 'FIZZY',
    'BOZOS', 'BONZE', 'PZAZZ', 'RAZZED', 'TUXEDO', 'VEXED', 'BOXED', 'HEXED'
  ],
};
...
export async function addWord(word: Omit<Word, 'id' | 'addedAt'>): Promise<Word> {
...
export async function getWordsByCategory(category: Word['category']): Promise<Word[]> {
  if (!db) await initWordsDB();
  if (!db) throw new Error('Failed to initialize DB');
...
export async function updateWord(id: string, updates: Partial<Word>): Promise<Word> {
  if (!db) await initWordsDB();
  if (!db) throw new Error('Failed to initialize DB');
...
export async function searchWords(query: string, category?: Word['category']): Promise<Word[]> {
  if (!db) await initWordsDB();
  if (!db) throw new Error('Failed to initialize DB');
